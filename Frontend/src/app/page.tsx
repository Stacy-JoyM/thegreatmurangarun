    ...
    <div className="overflow-x-hidden bg-white">
      ...
      <section className="relative">
        <div
          ...
          style={{ height: "clamp(380px, 92vw, 700px)" }}
        >
          <div
            ...
            <div className="max-w-3xl">
              <div className="flex items-center gap-2.5 mb-5">
                <Image
                  ...
                  alt="The Great Muranga Community Run"
                  width={26}
                  height={26}
                  className="h-6 w-6 rounded-full object-contain ring-2 ring-white/40"
                />
              ...
              <Link
                href="/registration"
                className="mt-6 inline-flex min-h-12 w-full sm:w-auto items-center justify-center gap-2 rounded-sm bg-white px-6 py-3.5 text-sm font-extrabold uppercase tracking-wide text-[color:var(--green)] transition hover:bg-[color:var(--yellow)]"
              >
                Register Free →
              </Link>
            </div>
      ...
      <section id="distance-section" className="bg-[color:var(--cream)]">
        <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-8 sm:py-16">
          ...
          <div className="grid grid-cols-1 items-stretch gap-3 md:grid-cols-2 xl:grid-cols-5">
            <CategoryCard num="01" distance="10km" title="Open"
              blurb="Open to all ages."
              level="All ages" accent="var(--green)" textAccent="var(--white)" featured />
            <CategoryCard num="02" distance="8km" title="Men Under 20"
              blurb="For young runners with pace and energy."
              level="Men · Under 20" accent="var(--ink)" textAccent="var(--white)" />
            <CategoryCard num="03" distance="2km" title="Youth"
              blurb="For runners aged 13 to 15."
              level="Ages 13 – 15" accent="var(--green-soft)" textAccent="var(--green)" />
            <CategoryCard num="04" distance="1km" title="Masters"
              blurb="For runners aged 50 to 60."
              level="Ages 50 – 60" accent="var(--green-soft)" textAccent="var(--green)" />
            <CategoryCard num="05" distance="500m" title="Seniors"
              blurb="Open to everyone aged 61 and above."
              level="Ages 61 and above" accent="var(--yellow)" textAccent="var(--green)"
        ...
        <div className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-8 sm:py-24">
          <div className="grid gap-8 md:grid-cols-[0.9fr_0.95fr_1.15fr] md:items-center md:gap-12">
            <div>
              ...
              >
                  <span className="text-[color:var(--green)]">For everyone in Muranga.</span>
              </h2>
            </div>
            <div
            ...
            <div className="space-y-4 text-[15px] leading-7 text-[color:var(--ink-soft)]">
              ...
              <Link
                href="/about"
                className="inline-flex items-center gap-1.5 text-sm font-bold uppercase tracking-wide text-[color:var(--green)] hover:underline"
              >
                About the run →
              </Link>
            </div>
      ...
      <section className="bg-white">
        <div className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-8 sm:py-24">
          ...
          <ul className="divide-y divide-[color:var(--hairline)]">
            ...
            <ScheduleRow time="07:00" title="Flag off" text="10km first, then 8km, 2km, 1km and 500m." highlight />
          ...
          <div className="mt-6">
            <Link href="/timeline" className="text-sm font-bold uppercase tracking-wide text-[color:var(--green)] hover:underline">
              Full timeline →
            </Link>
          </div>
        </div>
      ...
      <section className="bg-[color:var(--green)] text-white">
        <div className="mx-auto w-full max-w-6xl px-4 py-12 sm:px-8 sm:py-24">
          <div className="mb-10 sm:mb-12">
            <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-white/60">What to Expect</p>
            <h2 className="mt-3 font-extrabold uppercase leading-none tracking-tight text-[28px] sm:text-4xl">
              Race Day Energy
            </h2>
          </div>
      ...
      <section className="bg-[color:var(--yellow)]">
        <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-8 sm:py-14">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[color:var(--green)]">Join the first edition</p>
              <h2
                className="mt-2 font-extrabold uppercase leading-none tracking-tight text-[color:var(--green)]"
            ...
            <Link
              href="/registration"
              className="inline-flex min-h-12 w-full items-center justify-center rounded-sm bg-[color:var(--green)] px-8 py-4 text-center text-sm font-bold uppercase tracking-wide text-white transition hover:bg-[color:var(--green-dark)] sm:w-auto"
            >
              Register Now →
            </Link>
          </div>
...
function CategoryCard({ num, distance, title, blurb, level, accent, textAccent, featured = false, className = "" }: {
  num: string; distance: string; title: string; blurb: string;
  level: string; accent: string; textAccent: string; featured?: boolean; className?: string;
}) {
  return (
    <Link
      href={`/registration?category=${encodeURIComponent(distance)}`}
      aria-label={`Register for the ${distance} ${title} category`}
      className={`group relative flex h-full w-full min-w-0 overflow-hidden rounded-[20px] border border-black/5 p-4 shadow-[0_10px_30px_-18px_rgba(0,0,0,0.28)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_16px_36px_-20px_rgba(0,0,0,0.24)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--green)] focus-visible:ring-offset-2 sm:p-5 ${featured ? "ring-2 ring-[color:var(--green)]" : ""} ${className}`}
      style={{ background: accent, color: textAccent }}
    >
      <div className="absolute inset-0">
        <div className="absolute -right-10 -top-10 h-28 w-28 rounded-full bg-white/10 blur-2xl" />
        <div className="absolute right-4 top-10 text-[4.4rem] font-black leading-none opacity-[0.07] sm:text-[5rem]">
          {num}
        </div>
      </div>

      <div className="relative z-10 flex min-h-[170px] w-full min-w-0 flex-col sm:min-h-[180px]">
        <div className="flex items-center justify-between gap-2">
          <span className="text-[10px] font-bold uppercase tracking-[0.18em] opacity-55">
            {num}
          </span>
          {featured && (
            <span className="rounded-full bg-white/18 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] opacity-90">
              Popular
            </span>
          )}
        </div>
        ...
        <div className="mt-4">
          <p className="font-extrabold uppercase leading-none tracking-tight text-[2rem] sm:text-[2.8rem]">{distance}</p>
          <p className="mt-2 text-[15px] font-bold tracking-tight leading-snug break-words">{title}</p>
          <p className="mt-2 max-w-[28ch] text-[13px] leading-5 opacity-80">{blurb}</p>
        </div>

        <div className="mt-auto pt-4">
          <div className="flex items-center justify-between border-t border-white/15 pt-3 text-[11px] font-bold uppercase tracking-[0.16em]">
            <span className="opacity-70">{level}</span>
            <span className="inline-flex items-center gap-1 opacity-90">
              Register Now <span aria-hidden className="transition-transform duration-300 group-hover:translate-x-0.5">→</span>
            </span>
          </div>
        </div>
      </div>
...
function ScheduleRow({ time, title, text, highlight = false }: {
  ...
  return (
    <li className="grid grid-cols-[72px_1fr] items-baseline gap-4 py-4 sm:grid-cols-[110px_1fr] sm:gap-8">
      <p className={`text-sm font-extrabold tracking-tight sm:text-base ${highlight ? "text-[color:var(--green)]" : "text-[color:var(--ink)]"}`}>
        {time}
      </p>
      <div>
        <p className="flex flex-wrap items-center gap-2 text-[13px] font-bold text-[color:var(--ink)] sm:text-sm">
          {title}
          {highlight && (
            <span className="rounded-full bg-[color:var(--green-soft)] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[color:var(--green)]">
              Start
            </span>
          )}
        </p>
...
function Feature({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
    <div className="rounded-sm border border-white/10 bg-white/5 p-5 sm:border-0 sm:bg-transparent sm:p-0">
      <div className="flex h-11 w-11 items-center justify-center rounded-full bg-white/15 text-white ring-1 ring-white/20">{icon}</div>
